...
public:
    ...
    public:
        ...
        bool operator==(const Iterator& other) const {
            if (nodeStack.empty() && other.nodeStack.empty()) {
                return true; // оба на конце
            }
            if (nodeStack.empty() || other.nodeStack.empty()) {
                return false; // один на конце, другой нет
            }
            return nodeStack.top() == other.nodeStack.top(); // сравнение текущих узлов
        }
    ...
    static void testItImCrying() {
        ...
        assert(it != myTreeIsBigItsRealyRealyBig.end()); // Должен иметь хотя бы один элемент
        assert(*it == 1);         // Первый элемент должен быть 1
        ++it;
        assert(*it == 2);         // Второй элемент должен быть 2
        ++it;
        assert(*it == 3);         // Третий элемент должен быть 3
        ++it;
        assert(it == myTreeIsBigItsRealyRealyBig.end()); // Достигнуть конца

        // Тестирование верти крученых
        ...
        std::cout << "Я плак делать хочу :(" << std::endl;
    }
