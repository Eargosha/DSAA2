...
public:
    ...
    /// @brief Класс итератора для BST. Инордерный обход дерева - LNR.
    ...
    private:
        ...
        /// Стек для хранения
        stack<TreeNodule<Type>*> noduleStack;

        void pushLeft(TreeNodule<Type>* node)
        {
            while (node != nullptr)
            {
                noduleStack.push(node);
    ...
    public:
        ...
        {
            return !noduleStack.empty() || !other.noduleStack.empty();
        }
    ...
    static void assertIterator()
    {
        ...
        cout << endl;
        // For по коллекции
        // сокращенная вариация for (BinarySearchTree<int>::Iterator it = tree.begin(); it != tree.end(); ++it)
        // BinarySearchTree<int>::Iterator - Это есть обращение к вложенному итератору
        for (int value : tree)
        {
            cout << (value < 3) << ' ' << endl;
        }
        ...
        // For по коллекции
        // Вместо auto можно использовать BinarySearchTree<int>::Iterator
    ...
    static void doTest()
    {
        ...
        // Дерево для копии
        BinarySearchTree<int> treeToCopy;
        treeToCopy.insert(9);
        treeToCopy.insert(2);
        treeToCopy.insert(14);
        treeToCopy.insert(6);
        treeToCopy.insert(11);
        treeToCopy.insert(19);

        // Проверка копирования
        BinarySearchTree<int> coper;
        coper = treeToCopy.copy();
        vector<int> inOCopy = { 2, 6, 9, 11, 14, 19 };
        // перевести в вектор и сравнить !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        assert(coper.inorder() == inOCopy);
        ...
        myTree.applyInorder([](int& val)
            { val *= 0; });
        copyTree.applyPostorder([](int& val)
            { val *= 2; });
        copyTree2.applyPreorder([](int& val)
            { val *= 2; });
        ...
        // search кинет исключение /// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        try
        {
            pustoBlin.getNodeByValue(10);
            assert(false);
        }
        catch (const exception e)
        {
