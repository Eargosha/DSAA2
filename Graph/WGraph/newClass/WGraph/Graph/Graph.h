...
public:
    ...
    /// <summary>
    /// Поиск минимально возможного пути между двумя вершинами методом Дейкстры от заданной вершины. 
    /// Бросает исключение out_of_range если вершина не существует
    /// Для этой реализации: BigO - O(n^2 + E) = связный|несвязный - O(1) = пустой
    /// Для реализации с приоритетной очередью и map:
    ...
    vector<T> depthFirstSearch(const T& startVertex) const {
        ...
        vector<T> visitedToReturn;

        // Вызываем рекурсивную функцию для обхода
        depthFirstSearchRecursive(startPosition, visited, visitedToReturn);
        cout << endl;
        return visitedToReturn;
    }
    ...
    void depthFirstSearchRecursive(int position, vector<bool>& visited, vector<T>& visitedToReturn) const {
        // Помечаем вершину как посещенную 
        visited[position] = true;
        // Выводим вершину на экран 
        cout << vertices[position] << " ";
        visitedToReturn.push_back(vertices[position]);
        ...
        for (int i = 0; i < vertices.size(); ++i) {
            ...
            if (weight != 0.0 && !visited[i]) {
                depthFirstSearchRecursive(i, visited, visitedToReturn); // Рекурсивно вызываем функцию для соседней вершины
            }
        }
    }
    ...
    /// </summary>
    /// <param name="startVertex">Вершина с которой начинается обход</param>
    vector<T> breadthFirstSearch(const T& startVertex) const {
        // Находим позицию стартовой вершины в графе 
        int startPosition = getVertexIndexInMassiveOfVertices(startVertex);
        ...
        while (!q.empty()) {
            ...
            // Показываем
            cout << vertex << " ";
            visitedToReturn.push_back(vertex);
            ...
            for (size_t i = 0; i < vertices.size(); ++i) {
                if (adjacencyMatrix[index][i] != 0.0 && find(visited.begin(), visited.end(), vertices[i]) == visited.end()) {
                    q.push(vertices[i]);
    ...
    vector<double> bellmanFord(const T& start) {
        ...
        for (size_t i = 1; i < vertices.size(); ++i) {
                ...
                for (size_t v = 0; v < vertices.size(); ++v) {
                    ...
                    if (adjacencyMatrix[i][v] < numeric_limits<double>::infinity()) { 
                        // Если есть ребро и
                        // Если найдено более короткое расстояние до v через u, обновляем его
                        if (distances[i] + adjacencyMatrix[i][v] < distances[v]) {
                            distances[v] = distances[i] + adjacencyMatrix[i][v];
                        }
                    }
                }
